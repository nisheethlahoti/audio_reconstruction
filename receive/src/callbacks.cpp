...
static inline void check_majority(logger_t &logger) {
	...
	for (int i = 0; i < packet_versions[0].size(); ++i) {
		...
		if (match == num_versions / 2)
			...
			logger.log(majority_not_found_log(latest_packet_number, num_versions));
			return;
		}
	}
	...
	if (expected_crc == received_crc) {
		logger.log(packet_recovered_log(latest_packet_number, num_versions));
		write_packet(packet + useless_length + 12, latest_packet_number, logger);
	} else {
		logger.log(packet_not_recovered_log(latest_packet_number, num_versions));
	}
}
...
void receive_callback(raw_packet_t packet, logger_t &logger) {
	...
	if (packet_number > latest_packet_number) {
		if (!written) {
			if (validated) {
				logger.log(deferred_writing_log());
				write_packet(packet_versions[0].data() + useless_length + 12, latest_packet_number,
				             logger);
			} else if (num_versions > 2) {
				check_majority(logger);
			} else {
				logger.log(not_enough_packets_log(latest_packet_number, num_versions));
			}
		}
